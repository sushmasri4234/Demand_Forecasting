"""
Main application routes for demand forecasting (with authentication).
"""
import os
import pandas as pd
import numpy as np
...
def train_model(product_id):
    ...
    # Load weekly data
    weekly_file = os.path.join(current_app.config['UPLOAD_FOLDER'], 'weekly_data.csv')
    ...
    model_path = os.path.join(current_app.config['MODEL_FOLDER'], f'model_{product_id}.pkl')
...
def upload():
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            filepath = os.path.join(current_app.config['UPLOAD_FOLDER'], 'raw_data.csv')
            file.save(filepath)
            ...
            try:
                ...
                weekly_path = os.path.join(current_app.config['UPLOAD_FOLDER'], 'weekly_data.csv')
                weekly_df.to_csv(weekly_path, index=False)
                
                # Log audit event
                log_audit_event('data_uploaded', user_id=current_user.id,
                              details=f"Uploaded {len(weekly_df)} weekly records")
                
                flash(f'File uploaded successfully! Converted to weekly data with {len(weekly_df)} records.', 'success')
                return redirect(url_for('main.train_select'))
...
@main.route('/train_select')
@login_required
def train_select():
    """Select product for training"""
    weekly_file = os.path.join(current_app.config['UPLOAD_FOLDER'], 'weekly_data.csv')
    if not os.path.exists(weekly_file):
    ...
    return render_template('main/train_select.html', products=products)
...
def train():
    ...
    try:
        ...
        # Log audit event
        log_audit_event('model_trained', user_id=current_user.id,
                      details=f"Trained model for product {product_id}, MAE: {mae:.2f}")
...
def models():
    ...
    for model_file in model_files:
        model_path = os.path.join(current_app.config['MODEL_FOLDER'], model_file)
        model_data = joblib.load(model_path)
        models_info.append({
    ...
    return render_template('main/models.html', models=models_info)
...
def predict():
    ...
    if request.method == 'POST':
        ...
        # Log audit event
        log_audit_event('prediction_generated', user_id=current_user.id,
                      details=f"Generated {n_weeks}-week forecast for product {product_id}")
        
        # Convert to HTML table
        predictions_html = result_df.to_html(classes='table table-striped', index=False)
        
        return render_template('main/predict_result.html', 
                             product_id=product_id, 
    ...
    return render_template('main/predict.html', products=products)
...
def dashboard():
    ...
    model_path = os.path.join(current_app.config['MODEL_FOLDER'], f'model_{selected_product}.pkl')
